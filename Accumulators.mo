package Accumulators
  model IdealAccumulator "Accumulator model filled with ideal Nitrogen gas"
    ...
    parameter SI.Volume V_total = 0.01 "Volume of filled gas";
    // Ambient
    ...
    // Initialization
    // 初期の油量
    parameter SI.Volume V_oil_start(min = 0) "Start value of oil volume" annotation(
      Dialog(tab = "Initialization"));
    // Mass and energy balance, ports
    extends Modelica.Fluid.Vessels.BaseClasses.PartialLumpedVessel(final fluidVolume = V_oil, heatTransfer(surfaceAreas = {crossArea + 2 * sqrt(crossArea * pi) * level}), final initialize_p = false, final p_start = p_ambient, energyDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, massDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial);
    // 初期温度，圧力を設定値にする
    // ポート情報を削除
  protected
